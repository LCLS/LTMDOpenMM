...
"\n"
"extern \"C\" __global__ void kNMLUpdate1_kernel( int numAtoms, int paddedNumAtoms, float tau, float dt, float kT, float* posq, float* posqP, float4* velm, float4* force,\n"
...
"\n"
"extern \"C\" __global__ void kNMLLinearMinimize1_kernel( int numAtoms, int numModes, float4 *velm, float4 *force, float4 *modes, float *modeWeights ) {\n"
"	extern __shared__ float dotBuffer[];\n"
"	for( int mode = blockIdx.x; mode < numModes; mode += gridDim.x ) {\n"
...
"						 float4 *force, float4 *modes, float *modeWeights ) {\n"
"	// Load the weights into shared memory.\n"
"	extern __shared__ float weightBuffer[];\n"
"	for( int mode = threadIdx.x; mode < numModes; mode += blockDim.x ) {\n"
"		weightBuffer[mode] = modeWeights[mode];\n"
...
"\n"
"extern \"C\" __global__ void kNMLQuadraticMinimize1_kernel( int numAtoms, float4 *posqP, float4 *velm, float4 *force, float *blockSlope ) {\n"
