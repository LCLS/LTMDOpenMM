...
"\n"
"extern \"C\" __global__ void kNMLLinearMinimize1_kernel( int numAtoms, int numModes, float4 *velm, long long *force, float4 *modes, float *modeWeights ) {\n"
"	extern __shared__ float dotBuffer[];\n"
"	for( int mode = blockIdx.x; mode < numModes; mode += gridDim.x ) {\n"
...
"\n"
"			float fx = (float)force[atom] / (float)0x100000000;\n"
"			float fy = (float)force[atom+1*numAtoms] / (float)0x100000000;\n"
"			float fz = (float)force[atom+2*numAtoms] / (float)0x100000000;\n"
"			float4 m = modes[modePos];\n"
"\n"
...
"}\n"
"\n"
"extern \"C\" __global__ void kNMLLinearMinimize2_kernel( int numAtoms, int numModes, float invMaxEigen, float4 *posq, float4 *posqP, float4 *velm,\n"
...
"		f.y *= sqrtInvMass;\n"
"		f.z *= sqrtInvMass;\n"
"		posqP[atom] = make_float4( (float)force[atom] / (float)0x100000000 - f.x, (float)force[atom+numAtoms]/(float) 0x100000000 - f.y, (float) force[atom+2*numAtoms]/(float) 0x100000000  - f.z, 0.0f );\n"
"\n"
"		float4 pos = posq[atom];\n"
...
"\n"
"extern \"C\" __global__ void kNMLQuadraticMinimize1_kernel( int numAtoms, float4 *posqP, float4 *velm, long long *force, float *blockSlope ) {\n"
...
"			const float fy = (float)force[atom+1*numAtoms] / (float)0x100000000;\n"
"			const float fz = (float)force[atom+2*numAtoms] / (float)0x100000000;\n"
"\n"
