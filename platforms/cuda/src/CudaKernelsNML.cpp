...
CudaIntegrateNMLStepKernel::CudaIntegrateNMLStepKernel(std::string name, const Platform& platform, CudaPlatform::PlatformData& data) : IntegrateNMLStepKernel(name, platform),
        data(data), modes(NULL), modeWeights(NULL), minimizerScale(NULL) {
}

CudaIntegrateNMLStepKernel::~CudaIntegrateNMLStepKernel() {
    if (modes != NULL)
        delete modes;
    if (modeWeights != NULL)
        delete modeWeights;
...
void CudaIntegrateNMLStepKernel::initialize(const System& system, const NMLIntegrator& integrator) {
...
void CudaIntegrateNMLStepKernel::execute(ContextImpl& context, const NMLIntegrator& integrator, const double currentPE, const int stepType) {
    _gpuContext* gpu = data.gpu;
